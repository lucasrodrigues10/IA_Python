...
clf = ensemble.BaggingRegressor(n_estimators=5)
clf.fit(dados_treino, target_treino.values.ravel())
predicao = clf.predict(dados_treino)
resultado = target_treino.reset_index(drop=True).copy()
predicao = [float(i) for i in predicao]

print(clf.score(dados_validacao, target_validacao))
print(clf.score(dados_treino, target_treino))
print(sklearn.metrics.mean_squared_error(target_validacao, predicao))
